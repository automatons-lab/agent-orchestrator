...
The `reviewer` role is described in the [review loop guide](/docs/guides/review-loop#ao-native-reviewer). Each role block also takes `githubUser` (a login declared under top-level `identities:`) and `agentConfig.reasoningEffort` (Codex `model_reasoning_effort`, Claude `--effort`). A third role, `reviewer`, configures the AO-native review loop and must use a different identity than `worker`. Role blocks are usually set once under `defaults:` and only overridden per project — see [Global Defaults](/docs/configuration#global-defaults).
